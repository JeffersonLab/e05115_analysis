...
*     -	Revision 2.0	1999/07/02 11:30:00 jinghua
*     -	(JLiu)change all HMS to HNSS
*     -
...
*     
      logical HES_ABORT,HKS_ABORT,COIN_ABORT,gmc_abort
      character*132 HES_err,HKS_err,COIN_err,gmc_err
      Integer*4 roc,hit,multi
*     
*--------------------------------------------------------
*     
      err= ' '
...
*
      ...
      gtrig_raw_tot_hits = 0
      gtrig_tot_hits = 0
*     
*     for F1TDC
      Do roc=1,g_decode_maxrocs
         Do hit=1,g_f1_max_trig
            Do multi=1,g_f1_multihit
               g_f1_trigger_time(roc) = -1
            EndDo
         EndDo
      EndDo

      call e_clear_event(HES_ABORT,HES_err)
*     
      call h_clear_event(HKS_ABORT,HKS_err)
*     
      call C_clear_event(COIN_ABORT,COIN_err)
*     
*     *      call gmc_mc_clear(gmc_abort,gmc_err)
*     
      ABORT= HES_ABORT .or. HKS_ABORT .or. COIN_ABORT !.or. gmc_abort
*     
      IF(ABORT) THEN
         err= COIN_err
         call G_prepend(HKS_err,err)
