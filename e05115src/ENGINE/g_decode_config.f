...
*
      ...
      integer HPAREID                           ! Need a LUN handler?
...
*
      ...
      logical OK,text
*
      integer llen,lp,lpcom, lpeq, m,i                          ! Line pointers
      character*1 tab
...
*     Need to pass this file name as an argument or through a common block.
      open(unit=HPAREID,status='OLD',file=fname,err=999)
*
      tab = char(9)
      roc = -1
      slot = -1
      ...
      lastslot = -1
      N_lines_read= 0
      mask = INT(Z'FFF')                ! Default data mask
      bsubadd = 17                      ! Default LSB of channel field
*
      Do i=1,G_DECODE_MAXWORDS 
         g_decode_didmap(i) = -1
         g_decode_planemap(i) = -1
         g_decode_countermap(i) = -1
         g_decode_sigtypmap(i) = -1
      EndDo
...
*
         OK= .FALSE.
         error= ':error reading'
         read(HPAREID, '(a)',err=555,end=666) line
         OK= .TRUE.
         error= ' '
...
*     ! MAP file format: subadd, plane, element, signal
*     Some detectors only have signal 0, and omitted in the map
*     Initialize signal to 0 here
*     read(line(1:llen),'(4i15)') subadd, plane,counter,signal
                  signal = 0
                  read(line(1:llen),*,iostat=ios) subadd, plane,counter,signal           
                  If(OK .and. (roc.ne.lastroc.or.slot.ne.lastslot)) Then
                     if(g_decode_slotpointer(roc,slot).lt.0) then
...
c      Write(*,*) 'roc,slot,slotp,did,pl,co,sig=',roc,slot,
c     &                    g_decode_slotpointer(roc,slot)+subadd,
c     &                    did,plane,counter,signal
