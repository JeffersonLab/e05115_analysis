...
*
* Revision 2.0  1999/07/16 12:00:00  jinghua
* (JLiu) Change HMS to HNSS. 
*        Note: The variable names are still use ?hms.
*
      ...
      do while (i.le.evlen)
        ...
        if (mode.eq.0) then
          ...
          if (name(1:11).eq.'Beam Energy') then
            ebeam=typed_value
          else if (name(1:12).eq.'HNSS Momentum') then
            phms=typed_value
          else if (name(1:9).eq.'HNSS Angle') then
            thms=epics_value
