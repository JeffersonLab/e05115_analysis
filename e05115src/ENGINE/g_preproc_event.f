...
*
* Revision 2.0  1999/07/02 16:25:00 jinghua
* Comment out some include files. They are not needed at all.
*
...
*---------------------------------------------------------------------
      ...
      integer*4 test1,test3
      Real*4 test2

* make sure to include all the necessary include files
* so that the tests below can be carried out

C      INCLUDE 'hnss_data_structures.cmn'
C      INCLUDE 'sos_data_structures.cmn'
C      INCLUDE 'hms_scin_tof.cmn'
C      INCLUDE 'sos_scin_tof.cmn'
...
*     for test purpose. 
*
       test1 = int(gen_event_ID_number/2)
       test2 = Real(gen_event_ID_number)/2.0
c       Write(*,*) 'test1,2=',test1,test2

       if (gen_event_type .ge. 1 .and. gen_event_type .le. 4) then
          if(Real(test1)-test2 .eq. 0) then
             preprocessor_keep_event=1
          EndIF
       endif
