...
*     -   Modified 06-Jul-1999   Jinghua Liu for HNSS
*     
*     Revision 2.0   1999/07/06 10:54:00 jinghua
*     Change HMS to HNSS
*     
...
*--------------------------------------------------------
      ...
      logical bad_report,bad_HES,bad_HKS,bad_COIN,bad_HBK
      character*132 err_report,err_HES,err_HKS,err_COIN,err_HBK
      integer HPAREID
...
*--------------------------------------------------------
      ...
      if (g_bad_output_filename.ne.' ') then
         ...
         open(unit=HPAREID,file='bad.tmp',status='unknown')
      endif
      
*     -chance to flush any statistics, etc.
      call e_proper_shutdown(HPAREID,bad_HES,err_HES)
      
      call h_proper_shutdown(HPAREID,bad_HKS,err_HKS)
      
      call C_proper_shutdown(HPAREID,bad_COIN,err_COIN)
     ...
     $     g_report_output_filename.ne.' ') then
      ...
      ABORT= bad_HES .or. bad_HKS .or. bad_COIN .or. bad_HBK
     $     .or. bad_report
      err= ' '
      IF(ABORT) THEN            !assemble error message
         ...
         ElseIf(bad_COIN) Then
            err= err_COIN
         EndIf
         If(bad_HKS .and. err.NE.' ') Then
            call G_prepend(err_HKS//' &',err)
         ElseIf(bad_HKS) Then
            err= err_HKS
         EndIf
         If(bad_HES .and. err.NE.' ') Then
            call G_prepend(err_HES//' &',err)
         ElseIf(bad_HES) Then
            err= err_HES
         EndIf
