...
*     -   Modified 20-Jun-1999   Jinghua Liu for HNSS
*==   >==>		(1)	HNSS uses the same event type 1 as HMS.
*==   >==>		(2)	We don't have coincidence anymore, but event type 3 
*==   >			is still used for normal events which contain both SOS 
*==   >			and HNSS infomation.
*==   >==>		(3)	COIN reconstruction still there.
*     
*     Revision 2.10  2000/02/16 15:56:02 jinghua
*     (JLiu) Removed the entire "hack*" part
*     
*     Revision 2.00  1999/06/20 10:03:23 jinghua
*     (JLiu) Change HMS to HNSS reconstruction.
*     
...
*     
      ...
      ENDIF
*     
      call G_analyze_triggersupervisor(ABORT,err)
      IF(ABORT) THEN
         call G_add_path(here,err)
         RETURN
      ENDIF
...
*     
*     
      IF(gen_event_type .eq. 4 .or. gen_event_type .eq. 8) then !pedestal event
         call g_analyze_pedestal(ABORT,err)
         update_peds = .true.   !need to recalculate pedestals
         if (g_decode_pedana.eq.0)RETURN ! analyze pedestal
      ENDIF
*     
...
*     
      ...
      ENDIF
*     
*     -Beamline reconstruction and triggers
      IF(gen_event_type.eq.1.or.
     &     gen_event_type.eq.2.or.  
     &     gen_event_type .eq. 3  
     &     ) then               !HKS/HES/COIN trig
...
*     
*     -HKS reconstruction
*     We should do hks one to determine start time center.
*
      IF(
     &     gen_event_type .eq. 1.or.  
     &     gen_event_type .eq. 3.or.  
     &     gen_event_type .eq. 4  
     &     ) then               !HKS/COIN trig
         ...
         ENDIF
         ABORT= ABORT .or. FAIL
         hks_event_counter(1)=hks_event_counter(1)+hnspace_points_tot
         hks_event_counter(2)=hks_event_counter(2)+hntracks_fp
         hks_event_counter(3)=hks_event_counter(3)+hnphysics
      ENDIF
      
*     -HES reconstruction, 
      IF(
     &     gen_event_type .eq. 1.or.  !test GOGAMI 28Dec2010
     &     gen_event_type .eq. 2.or. 
     &     gen_event_type .eq. 3.or.  
     &     gen_event_type .eq. 4  
     &     ) then               !HES/COIN trig
         call e_reconstruction(FAIL,why)
         IF(err.NE.' ' .and. why.NE.' ') THEN
            call G_append(err,' & '//why)
         ELSEIF(why.NE.' ') THEN
            err= why
