...
*             2. Call Register routines for HNSS, SOS and coincidence.
*
...
*     Modified: 16-Jul-1999 Jinghua Liu for HNSS
*
* Revision 2.2 2004/03/02 Miyoshi
* change for E01-011
*
* Revision 2.1   2000/02/28 18:00:00  jinghua
* Added r_g_beam_ntuple and g_bm_nt_register for beam monitor ntuple.
*
* Revision 2.0   1999/07/16 10:25:00  jinghua
* Change HMS to HNSS
*
...
*
      ...
      call r_gen_data_structures        ! Contains both HNSS and SOS stuff

      call r_g_beam_ntuple
...
*
      ...
      EndDo
*
      call g_bm_nt_register(FAIL,why) ! E01-011
      ...
      ABORT= ABORT .or. FAIL 
*
      call e_register_variables(FAIL,why) ! HES 
      IF(err.NE.' ' .and. why.NE.' ') THEN
         call G_append(err,' & '//why)
      ELSEIF(why.NE.' ') THEN
         err= why
      ...
      ABORT= ABORT .or. FAIL 
*     
      call h_register_variables(FAIL,why) ! HKS
      IF(err.NE.' ' .and. why.NE.' ') THEN
         call G_append(err,' & '//why)
      ELSEIF(why.NE.' ') THEN
         err= why
