...
*     -calculate secondary scintillator and time of flight parameters
      ...
      ABORT= ABORT .or. FAIL
*     SDISPLAY*     If one_ev flag on, initialize 
*     the event display package
*     if(one_ev.ne.0) call one_ev_init  !One event display unit
*     
*     -read in Optical matrix elements
*     switch=1 RAYTRACE matrixfit ; 0(default)=ERIKA fit
c      if(h_recon_switch .eq. 1) then
         Call h_targ_trans_init_rt(FAIL,why,istat)
c      Else
c         call h_targ_trans_init(FAIL,why,istat)
c      EndIf
      if(FAIL) then
