...
* internal Variables,
      ...
      integer*4 num_yhits
      real*4 xt,yt
      real*4 y_pos(edc1max_hits_per_point)
      real*4 max_dist
*
*
...
* loop over all hits, find y of space point.
      do k = 1, ncham_hits
        if (k.ne.x_hit .and. k.ne.xp_hit) then !y-like hits
          y_pos(k) = ( wire_center(k)-xt*edc2xsp(ipln(k)) )/edc2ysp(ipln(k))
          yt = yt + y_pos(k)
