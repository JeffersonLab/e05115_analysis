...
*
      integer*4 space_point_hits(hmax_space_points,hmax_dc_hits)
     
*
*     local variables
      integer*4 point,startnum,finalnum,goodhit(hmax_dc_hits)
      integer*4 layer1,layer2,hit1,hit2,deltatime
      integer*4 hits(hmax_dc_hits),multipleflag(12)
      integer*4 hits_multi(hmax_dc_hits)
      ...
      integer*4 hitid(12,hmax_dc_hits)
      integer*4 ignoreflag(hmax_hits_per_point+2)
      integer*4 hdtmin(12),hdtmax(12),minindex(12),maxindex(12)
      integer*4 multiplehitid(12,hmax_dc_hits)
      integer*4 nmultiple(hmax_dc_hits),kicknum,multinum
      integer*4 multilayerflag(12),multinum_per_layer(12)
      integer*4 nlayer,multipletestflag,imul(12)
      integer*4 hits_per_layer(12),ihit_layer(12,hmax_dc_hits)
...
*
      ...
      do i=1,hmax_space_points_multi
         space_points_multi(i)=0
c           print*,space_points_multi(i)
      enddo
*     
      do i=1,hmax_hits_per_point+2 
         ignoreflag(i)=0
      enddo
...
cDK         print*, "evid,startnum",gen_event_ID_number,startnum
         ...
         multinum=1
         nlayer=0
c check which chamber is it     
         firstlayer=hdc_layer_num(space_point_hits(point,3))
         if(firstlayer.le.6) then
            loopu=1
            loopd=6
         elseif(firstlayer.le.12) then
            loopu=7
            loopd=12
         else
...
c         write(59,*)eventid,point,m-2,space_point_hits(point,m),hdc_layer_num(space_point_hits(point,m)),
c     +                 hdc_drift_time_pre(space_point_hits(point,m))
...
c         print*,loopu,loopd
         ...
         do j=3,startnum+1
            ...
            layer3 = hdc_layer_num(hit3)
            drifttime3 = hdc_drift_time_pre(hit3)  
            do k=j+1,startnum+2
               hit4 = space_point_hits(point,k)
               layer4 = hdc_layer_num(hit4)
               drifttime4 = hdc_drift_time_pre(hit4)
...
cDK               print*, "evid,startnum",gen_event_ID_number,startnum
               do j=3,startnum+2
                  ...
                  layer5 = hdc_layer_num(hit5)
                  drifttime5 = hdc_drift_time_pre(hit5)
...
c              print*,minindex(i),maxindex(i),i
               do k=3,startnum+2 
                  ...
                  layer6 = hdc_layer_num(hit6)
                  drifttime6 = hdc_drift_time_pre(hit6)
                  if(layer6.eq.i) then
...
c          print*,finalnum         
            do i=loopu,loopd                  
               do j=3,startnum+2
                  ...
                  layer7 = hdc_layer_num(hit7)
                  drifttime7 = hdc_drift_time_pre(hit7)             
            ...
            if(multipletestflag.gt.0.and.multinum.le.10.and.multinum.gt.0) then
               do i=loopu,loopd
                  do j=3,startnum+2
                     ...
                     layer9 = hdc_layer_num(hit9)
                     drifttime9 = hdc_drift_time_pre(hit9) 
                     if(layer9.eq.i) then
                        if(goodhit(j).gt.0) then
...
c          print*,pln,hdc_layer_num(space_point_hits(point,k+2)),space_point_hits(point,k+2),point            
...
c           print*,int(k/finalnum),multinum
               ...
               if(multinum*finalnum.gt.k) then 
                  space_points_multi(point)=int(k/finalnum)
               else
                  space_points_multi(point)=multinum
               endif
...
c             print*,space_point_hits(point,i),hdc_layer_num(space_point_hits(point,i)),point,nspace_points
c           enddo
c         print*,'eventid'
...
c      print*,space_points_multi(i),i,space_point_hits(i,1),hit1,hdc_layer_num(hit1)
c           
c          enddo
c        enddo
*******    
