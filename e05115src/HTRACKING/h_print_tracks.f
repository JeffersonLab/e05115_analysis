 ...
 1001       format(1x,i3,2x,4e14.6,e10.3,1x,i3)
     ...
     &           HNFREE_FP(itrack)
         ...
         do itrack=1,HNTRACKS_FP
            ...
            write(hluno,'(a,i3)') ' Hits in HKS track number',itrack
            write(hluno,*) '   hit  layer  HDC_WIRE_COORD',
     &           '   FIT POSITION    RESIDUAL'
*     
            do ihit=1,HNTRACK_HITS(itrack,1)
               hitnum=HNTRACK_HITS(itrack,ihit+1)
               layernum=HDC_LAYER_NUM(hitnum)
               calculated_position=H_DPSIFUN(ray,layernum)
               residual=dble(HDC_WIRE_COORD(hitnum))-calculated_position
               write(hluno,1011) hitnum,layernum,HDC_WIRE_COORD(hitnum),
