...
*     event, we just set the cointime to zero.
       ...
       if (gen_event_type.eq.3) then
          thiscointime=ccointime_hks
          if (thiscointime.eq.lastcointime) then
             thiscointime=100.0
          endif
          lastcointime=ccointime_hks
       endif
...
*     look at them with the event display to see if they're worrisome.
       if (hbypass_track_eff_files.eq.0) then
          open(unit=15,file='scalers/htrackeff.txt',status='unknown',
     $         access='append')
       endif
*     
...
*     statement at the end of this file.
       if (hbypass_track_eff_files.eq.0) then
          open(unit=17,file='scalers/htrack.out',status='unknown',
     $         access='append')
       endif
...
*     next, we see how many hits per plane there were ...
       ...
       hnumhith1 = HDC_HITS_PER_PLANE(1) + HDC_HITS_PER_PLANE(2) +
     $      HDC_HITS_PER_PLANE(3) + HDC_HITS_PER_PLANE(4) +
     $      HDC_HITS_PER_PLANE(5) + HDC_HITS_PER_PLANE(6) 
       hnumhith2 = HDC_HITS_PER_PLANE(7) + HDC_HITS_PER_PLANE(8) +
...
*     this number should agree with the value in trackeff.test.
       ...
       h1hitslt = hnumhith1.LE.hmax_pr_hits(1)
       h2hitslt = hnumhith2.LE.hmax_pr_hits(2)
...
*this number should agree with the value in trackeff.test.
       ...
       hnumplaneh2 = planetemp
       
       h1planesgt = hnumplaneh1.GE.hmin_hit(1)
       h2planesgt = hnumplaneh2.GE.hmin_hit(2)
...
*     we now fill in the chamber part of the track tests.
       ...
       hcleantrack = (hhnum_fptrack.NE.0).AND.(hhbeta.GT.(0.1))
...
*ss   pacepoints is finding at least one space point in both chambers
       hspacepoints = ((hnspace_points(1).GE.1).AND.(hnspace_points(2).GE.1))
*ss   tublt is passing the stub criteria for at least one spacepoint in both chambers
...
*     if a track should have been found.
       ...
       hnumscinh1 = hscin_hits_per_plane(1)
       hnumscinh2 = hscin_hits_per_plane(2)
       hnumscinh3 = hscin_hits_per_plane(3)
...
*Ww   e count the number of three adjacent scintillators too.  (A signle track 
...
*     look for clusters in first x plane... (9 scins)
       count=0
       if (hscinhit(1,1).EQ.1) count=count+1
       do i=1,(hscin_1x_nr-1)   !look for number of clusters of 1 or more hits
          if ((hscinhit(1,i).EQ.0).AND.(hscinhit(1,i+1).EQ.1)) count=count+1
       enddo
       hnclust(1)=count
       count=0
       do i=1,(hscin_1x_nr-2)   !look for three or more adjacent hits
          if ((hscinhit(1,i).EQ.1).AND.(hscinhit(1,i+1).EQ.1).AND.
     $         (hscinhit(1,i+2).EQ.1)) count=count+1
       enddo
       if (count.GT.0) hthreescin(1)=1
*     look for clusters in second x plane... (16 scins)
       count=0
       if (hscinhit(3,1).EQ.1) count=count+1
       do i=1,(hscin_2x_nr-1)   !look for number of clusters of 1 or more hits
          if ((hscinhit(3,i).EQ.0).AND.(hscinhit(3,i+1).EQ.1)) 
     &         count=count+1
       enddo
       hnclust(3)=count
       count=0
       do i=1,(hscin_2x_nr-2)   !look for three or more adjacent hits
          if ((hscinhit(3,i).EQ.1).AND.(hscinhit(3,i+1).EQ.1).AND.
     $         (hscinhit(3,i+2).EQ.1)) count=count+1
       enddo
       if (count.GT.0) hthreescin(3)=1
*     look for clusters in y planes... (9 scins)
       do j=2,4,2
          ...
          do i=1,(hscin_1y_nr-1) !look for number of clusters of 1 or more hits
             if ((hscinhit(j,i).EQ.0).AND.(hscinhit(j,i+1).EQ.1)) 
     &            count=count+1
          enddo
          hnclust(j)=count
          count=0
          do i=1,(hscin_1y_nr-2) !look for three or more adjacent sits
             if ((hscinhit(j,i).EQ.1).AND.(hscinhit(j,i+1).EQ.1).AND.
     $            (hscinhit(j,i+2).EQ.1)) count=count+1
          enddo
          if (count.GT.0) hthreescin(j)=1
       enddo
       if ((gen_event_ID_number.GT.1000).AND.((hnclust(2).GT.1).OR.
     $      (hthreescin(2).EQ.1))) then
          if (hbypass_track_eff_files.eq.0) then
             write(15,*) 'three or cluster in 1st yplane event',
     &            gen_event_ID_number
          endif
       endif
...
*     have a slope of zero.  If it moves one scin over from h1x to h2x it has an
...
*     
       if (hbypass_track_eff_files.eq.0) then
          if (hgoodscinhits.EQ.1) then
             write(15,*) 'sweet spot hit, event number           ',
     &            gen_event_ID_number
          endif
          if (.not.hhitslt) then
             write(15,*) 'too many hits, event number           ',
     &            gen_event_ID_number
          endif
     ...
     $            gen_event_ID_number
          ...
          if ((.not.hfoundtrack).AND.hhitsplanessps) then
             write(15,*) 'p sits/planes/sps, f track # = ',
     &            gen_event_ID_number
          endif
...
*     writing to that file, do't' worry about this.
       ...
       if (hbypass_track_eff_files.eq.0) then
          ...
          write(17,902) gen_event_ID_number,hnumhith1,hnumhith2,
     $         hnumhit1,hnumhit2,hnumhit3,hnumhit4,
     $         hnumhit5,hnumhit6,hnumhit7,hnumhit8,
     $         hnumhit9,hnumhit10,hnumhit11,hnumhit12,
     $         hnumplaneh1,hnumplaneh2,hnumscinh1,hnumscinh2,
     $         hnumscinh3,hnumscinh4,hnclust(1),hnclust(2),
     $         hnclust(3),hnclust(4),hthreescin(1),hthreescin(2),
     $         hthreescin(3),hthreescin(4),hbestxpscin,hbestypscin,
     $         hgoodscinhits,
     $             txtft,txtct,hntracks_fp,hbeta_notrk,htestbeta,
     $         hhchi2perdeg,
     $         hhdelta,thiscointime
          
 902      format(1x,i6,i4,i4,12(i4),2(i2),4(i3),8(i2),2(i4),
     $         i2,i2,i2,i2,
     $         f10.3,f9.3,
     $         f10.3,f10.3,f10.3)
*     
          close(15)             !closes "htrackeff.txt"
          close(17)             !closes "htrack.out"
       endif
