      subroutine select_space_points_hks(nspace_point_len,
     & nspace_points,space_points,space_point_hits,space_points_multi,
     & min_hits,min_combos,
     & easy_space_point)
...
* if easy_space_point, then the number of combos is not filled.
        if(space_point_hits(ploop,2).ge.min_combos.or.easy_space_point) then
         ...
         space_points_multi(space_point_count)=space_points_multi(ploop)
c            print*, space_points_multi(space_point_count)
            if(space_points_multi(ploop).gt.0) then
             space_hits_tot=space_points_multi(ploop)
     &                      *space_point_hits(ploop,1)
            else 
               space_hits_tot=space_point_hits(ploop,1)
            endif
        space_point_hits(space_point_count,1)=space_point_hits(ploop,1)
        space_point_hits(space_point_count,2)=space_point_hits(ploop,2)

            do hloop=1,space_hits_tot
              space_point_hits(space_point_count,hloop+2)=
     &             space_point_hits(ploop,hloop+2)
            enddo
          endif
        endif
      enddo
